...
export default function LiquidEther({
  ...
  useEffect(() => {
    ...
    // Helper to ensure canvas is absolutely positioned and behind children
...
`;
    ...
    class WebGLManager implements LiquidEtherWebGL {
      ...
      init() {
        if (!Common.renderer) return;
        // Ensure canvas is absolutely positioned and behind children
        styleCanvas(Common.renderer.domElement);
        // Append canvas as the first child so children are above it
        if (this.props.$wrapper.firstChild) {
          this.props.$wrapper.insertBefore(Common.renderer.domElement, this.props.$wrapper.firstChild);
        } else {
  ...
  return (
    <div
      ...
      style={style}
    >
      {/* Children are rendered above the absolutely positioned canvas */}
      <div style={{ position: 'relative', top: 150, zIndex: 1, width: '100%', height: '100%' }}>
        {children}
      </div>
    </div>
  );
